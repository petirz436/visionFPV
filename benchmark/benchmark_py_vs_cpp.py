...
Evaluates frame-by-frame latency, FPS, IoU accuracy, and position error against Ground Truth.
Exports CSV, JSON summary, 4-panel statistical PNG chart, and side-by-side visualization MP4 video.
"""
...
def run_benchmark(headless=False, save_video=False):
    ...
    csv_writer.writerow([
        ...
        "gt_x", "gt_y", "gt_w", "gt_h",
        "py_x", "py_y", "py_w", "py_h", "py_err", "py_iou", "py_ms", "py_status",
        "cpp_x", "cpp_y", "cpp_w", "cpp_h", "cpp_err", "cpp_iou", "cpp_ms", "cpp_status"
    ])
    ...
    while True:
        ...
        csv_writer.writerow([
            ...
            py_box[0], py_box[1], py_box[2], py_box[3], round(py_err, 2), round(py_iou, 3), round(py_ms, 2), py_status,
            cpp_box[0], cpp_box[1], cpp_box[2], cpp_box[3], round(cpp_err, 2), round(cpp_iou, 3), round(cpp_ms, 2), cpp_status
        ])

        py_log.append({"err": py_err, "iou": py_iou, "ms": py_ms, "status": py_status})
        cpp_log.append({"err": cpp_err, "iou": cpp_iou, "ms": cpp_ms, "status": cpp_status})

        # Render Legend OSD
        cv2.putText(display_frame, f"Python SiamRPN: {py_status} Latency:{py_ms:.1f}ms Err:{py_err:.1f}px", (20, 450), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)
        cv2.putText(display_frame, f"C++ SiamRPN   : {cpp_status} Latency:{cpp_ms:.1f}ms Err:{cpp_err:.1f}px", (20, 470), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    ...
    print("==========================================================")
    print(f"Python visionFPV3 : Success: {summary['python_visionFPV3']['success_rate_percent']:5.1f}% | Avg Err: {summary['python_visionFPV3']['mean_error_px']:5.1f}px | Speed: {summary['python_visionFPV3']['fps']:5.1f} FPS ({summary['python_visionFPV3']['mean_latency_ms']:5.1f}ms)")
    print(f"C++ vision_fpv3   : Success: {summary['cpp_vision_fpv3']['success_rate_percent']:5.1f}% | Avg Err: {summary['cpp_vision_fpv3']['mean_error_px']:5.1f}px | Speed: {summary['cpp_vision_fpv3']['fps']:5.1f} FPS ({summary['cpp_vision_fpv3']['mean_latency_ms']:5.1f}ms)")
    print("==========================================================")
...
def generate_comparison_plot():
    ...
    ax1.set_title("1. Frame Processing Latency (ms - Lower is Better)", fontsize=12)
    ...
    ax2 = axes[0, 1]
    targets = ["Python (visionFPV3)", "C++ (vision_fpv3)"]
    ...
    ax2.set_title("2. Average Execution Speed Comparison (FPS & Latency)", fontsize=12)
    ...
    # 3. Position Center Error Line Chart
    ax3 = axes[1, 0]
    ax3.plot(frames, df["py_err"].clip(upper=150), label="Python visionFPV3", color=c_py, linewidth=2.0)
    ax3.plot(frames, df["cpp_err"].clip(upper=150), label="C++ vision_fpv3", color=c_cpp, linewidth=2.0)
    ax3.axvspan(125, 175, color='#E74C3C', alpha=0.25, label="Occlusion Zone")
    ax3.set_title("3. Target Position Center Error (Pixels - Lower is Better)", fontsize=12)
    ...
    # 4. Overall Lock Success Rate Bar Chart
    ax4 = axes[1, 1]
    success_rates = [summary["python_visionFPV3"]["success_rate_percent"], summary["cpp_vision_fpv3"]["success_rate_percent"]]
    bars4 = ax4.bar(targets, success_rates, color=[c_py, c_cpp], alpha=0.85, width=0.4)
    ax4.set_title("4. Overall Target Lock Success Rate (%)", fontsize=12)
